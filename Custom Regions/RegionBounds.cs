...
namespace Server.Commands
{
    public class RegionBounds
    {
        ...
        private class RegionBoundTarget() : Target(-1, false, TargetFlags.None)
        {
            protected override void OnTarget( Mobile from, object targeted )
            {
                if( targeted is Mobile mobile )
                {
                    ...
                    from.SendMessage(
                        $"That Mobile's region is of type {r.GetType().FullName}, with a priority of {r.Priority.ToString()}."
                    );
