...
namespace Server.Items
{
    ...
    public class RegionControl : Item
    {
        ...
        public bool GetFlag(RegionFlag flag) => ((m_Flags & flag) != 0);
        ...
        public RegionControl(Rectangle2D rect): base(5609)
        {
            ...
            DoChooseArea(newrect.Start, newrect.End, this);
        ...
        public RegionControl(Rectangle3D rect): base(5609)
        {
            ...
            DoChooseArea(rect.Start, rect.End, this);
        ...
        public RegionControl(Rectangle3D[] rects): base(5609)
        {
            ...
            {
                DoChooseArea(rect3d.Start, rect3d.End, this);
            }
        ...
        public void UpdateRegion()
        {
            ...
            {
                if (this.RegionArea is { Length: > 0 })
                {
                    m_Region = new CustomRegion(this)
                    {
                        GoLocation = m_CustomGoLocation
                    };
                    m_Region.Register();
                }
                else
        ...
        {
            BoundingBoxPicker.Begin(m, (_, start, end) => CustomRegion_Callback(start, end, this));
        }

        private void CustomRegion_Callback(Point3D start, Point3D end, object state)
        {
            DoChooseArea(start, end, state);
        }

        private void DoChooseArea(Point3D start, Point3D end, object state)
        {
            var control = (RegionControl)state; // Cast the state to RegionControl
            var areas = new List<Rectangle3D>();

            if (control.m_RegionArea != null)
            {
                areas.AddRange(control.m_RegionArea);
            }
            ...
            control.m_RegionArea = areas.ToArray();
