...
namespace Server.Items
{
    public class TurboSlot : Item
    {
        ...
        private int m_ReelOne;
        private int m_ReelTwo;
        private int m_ReelThree;
        ...
        private int m_TotalSpins;

        //Mobile & timeout
        private Mobile m_InUseBy;
        ...
        private string m_ProfSymbols;
        private string[] m_ProfReel = { null, null, null };
        private string m_ProfPayTable;
        ...
        private bool m_RandomActivated;
        ...
        private bool m_CardClubOnly; //Only club members can play this slot
        ...
        public Item ProgSlotMaster
        {
            ...
            set
            {
                if (m_isProgMaster)
                    return;
                if (value is TurboSlot && this != value && !value.Deleted)
                {
                ...
                {
                    if (m_ProgressiveMaster is { Deleted: false })
        ...
        public override void OnMovement(Mobile m, Point3D oldLocation)
        {
            if (m_InUseBy != null)
            {
                ...
                {
                    m_InUseBy.CloseGump<TurboSlotGump>();
#if !RUNUO2RC1
        ...
        public override void OnDoubleClick(Mobile from)
        {
            ...
            else
            {
                if (m_IdleTimer < m_TimeOut)
                {
                    ...
                    if (from != m_InUseBy)
                        from.SendMessage($"{m_InUseBy.Name} has left this machine idle too long, it is yours to play.");
            ...
            {
                if (m_InUseBy is { Name: not null })
                {
                    string text = $"{m_InUseBy.Name} is currently using this machine.";
                    from.SendMessage(text);
                }
                else
                {
                    from.SendMessage("This machine is currently in use by an unknown player.");
                }
            }
        }
        ...
        private bool CarryingClubCard(Mobile m)
        {
            ...
            for (int i = 0; i < packlist.Count; ++i)
            {
                ...
                if (item is { Deleted: false } and CasinoMembershipCard)
                {
        ...
        public bool CashCheck(Mobile m, out int amount)
        {
            ...
            {
                Item item = packlist[i];
                if (item is { Deleted: false } and BankCheck)
                {
        ...
        public string[] GetJackpotPayoutStr(int index, out decimal payout)
        {
            ...
            if (m_jackpotmultiplier[index] >= 0 && index is <= 10 and > -1)
            {
                if (index == 0)
                {
                    int p = m_Cost * m_jackpotmultiplier[0];
                    if ((m_ProgressiveMaster is { Deleted: false } && ((m_ProgressiveMaster as TurboSlot)?.ProgIsMaster == true)) || m_isProgMaster)
                    {
                        if (m_isProgMaster)
                        {
                            p += m_ProgressiveJackpot;
                        }
                        else if (m_ProgressiveMaster is TurboSlot masterSlot)
                        {
                            p += masterSlot.ProgJackpot;
                        }
                    }
        ...
        public void JackpotHit(Mobile from, int jackpot, TurboSlot slot)
        {
            ...
            Effects.SendLocationEffect(new Point3D(X, Y, Z), Map, 0x375A, 15, Hue);
            string text = $"{from.Name} has won a Progressive Jackpot valued at {jackpot} gold!!!";
            ...
            if (m_SlotSlaves != null)
            {
                ...
                foreach (TurboSlot s in m_SlotSlaves)   // Announce the jackpot, but not to every machine to avoid a messy screen
                {
                    ...
                    Effects.SendLocationEffect(new Point3D(s.X, s.Y, s.Z), s.Map, 0x375A, 15, Hue);
                }
            }
        }
        ...
        public void AddSlave(TurboSlot s)
        {
            if (m_SlotSlaves == null)
            {
                m_SlotSlaves = new ArrayList();
            }

            if (!m_SlotSlaves.Contains(s))
            {
                m_SlotSlaves.Add(s);
            }
        }
        ...
        public void DoSpin(Mobile from)
        {
            ...
            TurboSlot masterSlot = null;
            if ((m_ProgressiveMaster is { Deleted: false } && (masterSlot = m_ProgressiveMaster as TurboSlot)?.ProgIsMaster == true) || m_isProgMaster)
            {
                if (m_ProgressivePercent != 0)
                {
                    ...
                    else if (masterSlot != null)
                    {
                        masterSlot.AddToJackpot((int)amount);
                    }
                }
            }
                        ...
                        case 0:
                            {
                                ...
                                if ((m_ProgressiveMaster is { Deleted: false } && ((TurboSlot)m_ProgressiveMaster).ProgIsMaster) || m_isProgMaster)
                                {
                                    if (m_isProgMaster)
                                    {
                                        p += m_ProgressiveJackpot;
                                    ...
                                    {
                                        if (m_ProgressiveMaster is TurboSlot progressiveMaster)
                                        {
                                            p += progressiveMaster.ProgJackpot;

                                            if (!m_GiveReward)
                                                payout += progressiveMaster.ProgJackpot;

                                            var soundIndex = m_InUseBy is { Female: true } ? m_FemaleSounds[r1index] : m_MaleSounds[r1index];

                                            DoSpinWin(from, r1index, p, soundIndex, m_Sounds[r1index], m_JackpotText[r1index], m_JackpotEffect[r1index]);

                                            ActivateBlinkTimer(0);

                                            progressiveMaster.JackpotHit(from, (int)payout, this);
                                        }
                                    }
                                ...
                                {
                                    string text = $"{from.Name} has won a Jackpot valued at {payout} gold!!!";
                                    AnnounceJackpot(from, text);
                                }
                                DoSpinWin(from, r1index, p, m_InUseBy.Female ? m_FemaleSounds[r1index] : m_MaleSounds[r1index], m_Sounds[r1index], m_JackpotText[r1index], m_JackpotEffect[r1index]);
                                ActivateBlinkTimer(0);
            ...
            if ((m_Won > m_CreditCashOut || !m_Active) && m_InUseBy is { AccessLevel: < AccessLevel.Counselor })
            {
                DoCashOut(from);
            }
        ...
        private void DoSpinWin(Mobile from, int r1index, int winnings)
        {
            ...
            string message = $"You win {winnings} Gold!";
            from.SendMessage(38, message);
        }
        ...
        private int CountScatter(int scatterReel, int reelOne, int reelTwo, int reelThree)
        {
            if (m_ScatterPay == ScatterType.None)
                return 0;
            if (m_ScatterPay == ScatterType.LeftOnly)
            {
                if (scatterReel == reelOne && scatterReel == reelTwo)
                    return 2;
                if (scatterReel == reelOne)
                    return 1;
                return 0;
            }
            int count = 0;
            if (scatterReel == reelOne)
                count++;
            if (scatterReel == reelTwo)
                count++;
            if (scatterReel == reelThree)
                count++;
            return count;
        }
                ...
                case SlotThemeType.TailorTreats:
                    {
                        ...
                        if (jackpotindex is >= 0 and <= 2 && m_Rewards == JackpotRewardType.RewardAndCash && Utility.Random(100) > chance[jackpotindex])
                        {
                            return false;
                        }
                    }
        ...
        private void GiveItem(Mobile to, Item i)
        {
            ...
            if (pack != null)
            {
                ...
                {
                    text = $"{(i.Amount > 1 ? "Some" : "A")} {i.Name} has been placed in your backpack!";
                }
                else
                {
                    to.BankBox.DropItem(i);
                    text = $"{(i.Amount > 1 ? "Some" : "A")} {i.Name} has been placed in your bankbox!";
                }
        ...
        private void FlameStrike()
        {
            try
            {
                Effects.SendLocationEffect(new Point3D(X, Y, Z + 1), Map, 0x3709, 15, Hue - 1);
        ...
        private void Sparkle(int JackpotEffect)
        {
            try
            {
                if (JackpotEffect == -2)
                {
                    Effects.SendLocationEffect(new Point3D(X, Y, Z + 1), Map, 0x375A, 15, Hue - 1);
                    Effects.PlaySound(new Point3D(X, Y, Z), Map, 0x213);
                }
                else
                {
                    Effects.SendLocationEffect(new Point3D(X, Y + 1, Z), Map, 0x373A, 15, Hue - 1);
                    Effects.SendLocationEffect(new Point3D(X + 1, Y, Z), Map, 0x373A, 15, Hue - 1);
                    Effects.SendLocationEffect(new Point3D(X, Y, Z - 1), Map, 0x373A, 15, Hue - 1);
                }
            }
            catch
            {
        ...
        public void DoCashOut(Mobile from)
        {
            ...
            if (m_Won < 0) // This should never happen but protect against some kind of overflow and a wild payout
            {
                ...
                {
                    from.SendMessage($"Invalid gold won amount({m_Won}), reset to 0.");
                    m_Won = 0;
                }
            ...
            if (m_Won < 1000)
            {
                try
                {
                    ...
                    from.SendMessage($" {credit} gold has been added to your pack.");
                }
                catch
                {
            ...
            else
            {
                try
                {
                    ...
                    from.SendMessage($" A bank check for {credit} gold has been placed in your pack.");
                }
                catch
                {
                    from.SendMessage("There's a problem returning your gold, this slot machine is offline. Page for help.");
            ...
            if (credit >= 10000)
            {
                string text = $"{m_InUseBy.Name} is cashing out {credit} Gold!";
                PublicOverheadMessage(0, Hue, false, text);
            }
        ...
        private void SlotOffline(int error)
        {
            ...
            if (m_InUseBy != null && m_InUseBy.Map != Map.Internal)
            {
                try
                {
                    m_InUseBy.CloseGump<NewMinerBonusGump>();
                    m_InUseBy.CloseGump<TurboSlotPayTableGump>();
...
#endif
                default:
                    string text = $"Unknown Theme ({m_SlotTheme})! Slot Machine Offline.";
                    PublicOverheadMessage(0, Hue, false, text);
                    SlotOffline(8006);
                    break;
            }
        ...
        private void CreateOddsTable(int table, int[] distTable)
        {
            ...
            {
                string text =
                    $"Your distribution table needs to have {m_TotalSymbols}(It has {distTable.Length}) values! Slot Machine Offline.";
                PublicOverheadMessage(0, Hue, false, text);
                SlotOffline(8001);
                return;
            }
            ...
            {
                string text =
                    $"{m_PaybackType}. Distribution table {table} must total 100 or 64(It totals {maxRoll})! Slot Machine Offline.";
                PublicOverheadMessage(0, Hue, false, text);
                SlotOffline(8002);
                return;
            }
        ...
        private class InternalTimer1 : Timer
        {
            ...
            {
                m_slot = slot;
                DelayCall(delay, OnTick);
            }

            protected override void OnTick()
            {
                if (m_slot is { Deleted: false })
                {
        ...
        private class InternalTimer2 : Timer
        {
            ...
            protected override void OnTick()
            {
                ...
                DelayCall(TimeSpan.FromSeconds(1), () => new InternalTimer2(m_slot, TimeSpan.FromSeconds(1)).Start());
            }
        }
        #endregion
        ...
        public void GetProperties(ObjectPropertyList list)
        {
            ...
            if (!m_Active)
            {
                if (m_ErrorCode == 0)
                    ...
                    list.Add(1060658, $"Status\tMaintenance Required({m_ErrorCode})");
                return;
            }
            ...
            else
            {
                ...
                list.Add(1060659, $"Player\t{m_InUseBy.Name}");

                numberpos = 1060660;
            }
            if (m_LastWonBy != null)
            {
                list.Add(numberpos, $"Last Jackpot\t{m_LastWonBy.Name}");
                numberpos++;
                list.Add(numberpos, $"Date\t{m_LastWonByDate}");
                numberpos++;
                list.Add(numberpos, $"Amount\t{m_LastWonAmount}");
                ...
                case 0: // Release 0
                    {
                        ...
                        m_ProgressiveMaster = reader.ReadEntity<Item>();
    ...
    public class CasinoMembershipCard : Item   // Private Club "Jackpot Winners" Membership card
    {
        ...
        {
            base.GetProperties(list);
            if (m_Member is { Deleted: false })
            {
                list.Add(1060658, $"Member Name\t{m_Member.Name}"); // MemberName
                list.Add(1060659, $"Joined\t{m_JoinDate}");         // Date Jackpot Won
                if (m_Game != null)
                {
                    list.Add(1060660, $"Game\t{m_Game}");                            // Slot Machine
                    list.Add(1060661, $"Jackpot Value\t{m_Jackpot:##,###,##} Gold"); // Jackpot Amount
                }
        ...
        public override void Deserialize(IGenericReader reader)
        {
            base.Deserialize(reader);
            int version = reader.ReadInt();
            m_Member = reader.ReadEntity<Mobile>();
