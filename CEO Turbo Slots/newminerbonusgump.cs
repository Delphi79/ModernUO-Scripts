    ...
    public class NewMinerBonusGump : Gump
    {
        ...
        public NewMinerBonusGump( TurboSlot Slot, int[] Symbols, bool initialize, bool[] b, int[] c, int totalwon ) : base( 25, 25 )
        {
            ...
            for (int i = 0; i < 9; i++)
            {
                if (!b[i])
                {
                    AddButton(buttonx[i], buttony[i], 2117, 2118, i + 1);
#if TestMode
                    if (m_Slot.TestMode)
        ...
        public override void OnResponse(NetState state, in RelayInfo info)
        {
            ...
            if (m_c[buttonindex] == 0)
            {
                ...
                {
                    string text = $"Bonus Round: {m_TotalWon} gold!";
            ...
            else
            {
                ...
                from.SendMessage($"You have won {m_c[buttonindex]}, pick another spot to mine.");
