...
namespace Server.Gumps
{
    ...
    public class SeasonChange : Gump
    {
        ...
        {
            CommandSystem.Register("SeasonChange", AccessLevel.Counselor, new CommandEventHandler(SeasonChange_OnCommand));
            CommandSystem.Register("SC", AccessLevel.Counselor, new CommandEventHandler(SeasonChange_OnCommand));
        }
        ...
        public override void OnResponse(NetState sender, in RelayInfo info)
        {
            ...
            if (mapNameEntry != null)
                MapNameEntry = mapNameEntry.Trim();
            ...
            foreach (var ns in NetState.Instances)
            {
                ...
                if (m != null && m.Map == _map)
                {
                    ...
                    //ns.Send(PlayerPackets.SeasonChange.Instantiate(m.GetSeason(), true)); // Ensure the correct packet is sent
                    ns.SendMobileUpdate(m); // Ensure this method exists and sends the correct update packet
                }
            }
            from.CloseGump<SeasonChange>();
            from.SendGump(new SeasonChange(from, _map));
